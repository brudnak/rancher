...
func (na *NodeAnnotationsTestSuite) SetupSuite() {
	...
	na.ctxAsAdmin, err = na.client.WranglerContext.DownStreamClusterWranglerContext(clusterID)
	require.NoError(na.T(), err)
...
func (na *NodeAnnotationsTestSuite) TestRBACAnnotations() {
	...
	for _, tt := range tests {
		...
		na.Run(fmt.Sprintf("Testing annotations with role %s", tt.role), func() {
			...
			testID := uuid.New()[:8]
			annotationKey := fmt.Sprintf("test-rbac-%s", testID)
			annotationValue := fmt.Sprintf("test-value-%s", testID)
			...
			for retries := 0; retries < 3; retries++ {
				...
				if updateErr == nil || !strings.Contains(updateErr.Error(), "401") {
					break
				}
			...
			if tt.canModify {
				require.NoError(na.T(), updateErr)
...
func (na *NodeAnnotationsTestSuite) TestNodeAnnotationsWithTableTests() {
	...
	testID := uuid.New()[:8]
	updateKey1 := fmt.Sprintf("test-multi-update-1-%s", testID)
	updateKey2 := fmt.Sprintf("test-multi-update-2-%s", testID)
	mixedKey1 := fmt.Sprintf("test-mixed-1-%s", testID)
	mixedKey2 := fmt.Sprintf("test-mixed-2-%s", testID)
	cycleKey := fmt.Sprintf("test-cycle-%s", testID)
	multiEditKey := fmt.Sprintf("test-multi-edit-%s", testID)
	...
	tests := []struct {
	...
	}{
		{
			...
			annotations: map[string]string{
				fmt.Sprintf("test-single-%s", uuid.New()[:8]): "simple-value",
			},
			operation: "add",
		},
		{
			...
			annotations: map[string]string{
				fmt.Sprintf("test-multi-1-%s", uuid.New()[:8]): "value1",
				fmt.Sprintf("test-multi-2-%s", uuid.New()[:8]): "value2",
				fmt.Sprintf("test-multi-3-%s", uuid.New()[:8]): "value3",
			},
			operation: "add",
		},
		{
		...
		},
		{
			...
			annotations: map[string]string{
				fmt.Sprintf("test-long-%s", uuid.New()[:8]): strings.Repeat("very-long-value", 50),
			},
			operation: "add",
		},
		{
			...
			annotations: map[string]string{
				fmt.Sprintf("test-special-%s", uuid.New()[:8]): "value with spaces!@#$%^&*()",
			},
			operation: "add",
		},
		{
			...
			annotations: func() map[string]string {
				...
				bulkID := uuid.New()[:8]
				for i := 0; i < 50; i++ {
					key := fmt.Sprintf("test-bulk-%d-%s", i, bulkID)
